...
df_rr <- df1 %>% dplyr::filter(!stage_mode %in% c('auto_rickshaw', 'bus', 'truck'))
df_rr <- tidyr::spread(data = df_rr, key = stage_mode, value = value)

readr::write_csv(df_rr, 'results/multi_city/whw_matrices/injury_risks_per_billion_kms_by_scen.csv')
...
d2 <- ggplot(df %>% dplyr::filter(!stage_mode %in% c('auto_rickshaw', 'bus', 'truck')), aes(x = stage_mode, fill = variable, weight = value)) +
    geom_bar(position = "dodge", colour = 'black') +
    scale_fill_manual(values = cols_cbrewer) +
    theme_minimal() + 
