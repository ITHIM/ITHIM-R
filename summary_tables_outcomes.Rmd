...
Click here: [change in yll](#change_in_yll_100k)
...
### Change in deaths due to injury
...
### Change in deaths due to injury per 100,000
...
### Change in deaths due to PA per 100,000
```{r}
...
### Change in deaths due to AP per 100,000
```{r}
...
### Change in YLLs due to injury per 100,000
...
### Change in YLLs due to PA per 100,000
```{r}
pa_rates <- lapply(cities,function(x) rbind(pa_totals[[x]]/t(repmat(pop_by_age[[x]],5,1))*100000, Total=colSums(pa_totals[[x]])/rep(sum(pop_by_age[[x]]),length=5)*100000))
names(pa_rates) <- cities
...
### Change in YLLs due to injury AP 100,000
```{r}
ap_rates <- lapply(cities,function(x) rbind(ap_totals[[x]]/t(repmat(pop_by_age[[x]],5,1))*100000, Total=colSums(ap_totals[[x]])/rep(sum(pop_by_age[[x]]),length=5)*100000))
names(ap_rates) <- cities
...
### Change in deaths due to disease
...
### Change in deaths due to disease per 100,000
...
## Change in YLL due to disease per 100,000 {#change_in_yll_100k}
