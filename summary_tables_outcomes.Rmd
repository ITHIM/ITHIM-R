...
Who hit whom matrix for baseline for each case study

### Who hit whom matrix for baseline {#whw_baseline}

```{r whw}

for(city in cities){
  if (!city %in% c('buenos_aires', 'mexico_city')){
    for (cs in names(io$accra$outcomes$whw)[1]){
      print(kable(format(round(io[[city]]$outcomes$whw[[cs]]$whw)), caption = paste('Who Hit Whom (WHW) ', cs, ' matrix for ', city)))
      cat("\n")
    }
...
No Other Vehicle matrix for baseline for each case study

### No Other Vehicle matrix for baseline {#nov_baseline}

```{r nov}

for(city in cities){
  if (!city %in% c('bogota', 'vizag')){
    for (cs in names(io$accra$outcomes$whw)[1]){
      print(kable(format(round(io[[city]]$outcomes$whw[[cs]]$nov)), caption = paste('NOV ', cs, ' matrix for ', city)))
      cat("\n")
    }
...
### Change in deaths due to injury per 100,000K
...
for (city in cities){
  ...
  el <- list()
    ...
    for (cs in names(io[[city]]$outcomes$whw)){
      ...
      td4 <- io[[city]]$dist %>% filter(stage_mode %in% td3$mode) %>% dplyr::select(stage_mode, cs)
