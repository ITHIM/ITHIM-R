...
```{r whw_for_all_cs, fig.width=7, fig.height=4, echo=FALSE}
...
for (city in cities){
  ...
  el <- list()
    ...
    for (cs in names(io[[city]]$outcomes$whw)){
      ...
      td4[, 2] <- round((td4[,3] / ( td4[,2] * 52)) * 100000, 4)
...
for (city in cities){
  ...
  el <- list()
    ...
    for (cs in names(io[[city]]$outcomes$whw)){
      ...
      td4[, 3] <- round(td4[, 3] / sum(io[[city]]$demographic$population) * 100000, 2)
      # / sum(io[[city]]$demographic$population, na.rm = T)
      
      names(td4)[2] <- paste(names(td4)[2], names(td4)[3], sep = "_")
