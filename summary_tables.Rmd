...
# Script summarises mode splits, distances and durations travelled and looks at health outcomes (YLLs and deaths) for the different air pollution, injury and physical activity pathways but also by disease

# WARNING: This script only works if ITHIM-Global has been run in constant mode!
...
# - one table for each city showing the change in total YLLs in the scenarios compared to the Baseline for each age group

# - one table for each city showing the change in YLLs per 100,000 people in the scenarios compared to the Baseline for each age group

# - one table for each city showing the change in YLLs per 100,000 people in the scenarios compared to the Baseline for each age and sex category

# - one table for each city showing the change in total YLLs due to changes in accident fatalities in the scenarios compared to the Baseline for each age group

# - one table for each city showing the change in total YLLs due to changes in physical activity levels in in the scenarios compared to the Baseline for each age group

# - one table for each city showing the change in total YLLs due to changes in air pollution (PM2.5) levels in in the scenarios compared to the Baseline for each age group

# - one table for each city showing the change in YLLs per 100,000 people due to changes in accident fatalities in the scenarios compared to the Baseline for each age group

# - one table for each city showing the change in YLLs per 100,000 people due to changes in physical activity levels in in the scenarios compared to the Baseline for each age group

# - one table for each city showing the change in YLLs per 100,000 people due to changes in air pollution (PM2.5) levels in in the scenarios compared to the Baseline for each age group


## Health outcomes - disease
# - one table for each city showing the change in deaths in the scenarios compared to the Baseline for each disease

# - one table for each city showing the change in deaths per 100,000 people in the scenarios compared to the Baseline for each disease

# - one table for each city showing the change in YLLs in the scenarios compared to the Baseline for each disease

# - one table for each city showing the change in YLLs per 100,000 people in the scenarios compared to the Baseline for each disease
...
# - the ylls.csv file (results/multi_city/health_impacts/ylls.csv) gives the changes in YLLs for each age and sex category, for each age category per 100,000 people, and for each age and sex category per 100,000 people for each city, disease and scenario. It also shows the disease level, whether the disease is exasperated by changes in AP, PA or both, and the age and age and sex specific population levels. The results are for either AP and PA only if the diseases are affected by only AP or PA or the results for the combined AP and PA effects if the diseases are affected by both AP and PA. 

# - the ylls_pathway.csv file (results/multi_city/health_impacts/ylls_pathway.csv) gives the changes in YLLs for each age and sex category, for each age category per 100,000 people, and for each age and sex category per 100,000 people for each city, disease and scenario. It also shows the disease level, whether the disease is exasperated by changes in AP or PA, and the age and age and sex specific population levels. The results are given for AP and PA independently even if a disease is affected by both pathways.
...
# - set up helper function to re-format large numbers, re-name scenarios and format and print output tables
...
# - create one table for each city showing the change in total YLLs in the scenarios compared to the Baseline for each age group

# - create one table for each city showing the change in YLLs per 100,000 people in the scenarios compared to the Baseline for each age group

# - create one table for each city showing the change in YLLs per 100,000 people in the scenarios compared to the Baseline for each age and sex category

# - create one table for each city showing the change in total YLLs due to changes in accident fatalities in the scenarios compared to the Baseline for each age group

# - create one table for each city showing the change in total YLLs due to changes in physical activity levels in in the scenarios compared to the Baseline for each age group

# - create one table for each city showing the change in total YLLs due to changes in air pollution (PM2.5) levels in in the scenarios compared to the Baseline for each age group

# - create one table for each city showing the change in YLLs per 100,000 people due to changes in accident fatalities in the scenarios compared to the Baseline for each age group

# - create one table for each city showing the change in YLLs per 100,000 people due to changes in physical activity levels in in the scenarios compared to the Baseline for each age group

# - create one table for each city showing the change in YLLs per 100,000 people due to changes in air pollution (PM2.5) levels in in the scenarios compared to the Baseline for each age group

# - create one table for each city showing the change in deaths in the scenarios compared to the Baseline for each disease

# - create one table for each city showing the change in deaths per 100,000 people in the scenarios compared to the Baseline for each disease

# - create one table for each city showing the change in YLLs in the scenarios compared to the Baseline for each disease

# - create one table for each city showing the change in YLLs per 100,000 people in the scenarios compared to the Baseline for each disease

# - create 4 individual .csv files in (results/multi_city/health_impacts/) for both death and YLLs considering the effect of AP and PA separately (*_pathway.csv) or together (*.csv). These files give the changes in YLLs / deaths for each age and sex category, for each age category per 100,000 people, and for each age and sex category per 100,000 people for each city, disease and scenario. They also show the disease level, whether the disease is exasperated by changes in AP, PA or both, and the age and age and sex specific population levels. 
...
# find reference scenario
ref_scen <- io[[cities[1]]]$ithim_run$reference_scenario
reference_scenario <- get_qualified_scen_name(ref_scen) # update scen name
...
get_scen_settings <- ithimr::get_scenario_settings(cities)
...
## Distance tables
City specific distance tables showing average distance in km travelled by each person by mode for all scenarios (incl Baseline)


```{r trip_dist = "asis"}
# find the average distance a person spends travelling on each mode per day
...
for (city in cities) { # loop through cities
  ...
  # update scenario names - cycle, car, bus
  ...
  print(kable(td, caption = paste("Distance table (km) for ", city, "( ", count_people, " ) per person (people with trips ONLY)")))
...
### Distance table showing average baseline distances in km travelled by each person by mode for all cities

```{r}
# create table showing average baseline distances by mode for all cities
trip_dist <- trip_dist %>% janitor::adorn_totals('row')

kable(trip_dist, caption = "Avg. baseline distance per person (km) per day across all cities")
...
City specific distance tables for baseline and scenarios for the entire population with age range considered in the model
...
for (city in cities) { # loop through cities
  ...
  # find the total city level population considered in the model only
...
# create baseline yearly distance table for all cities
...
for (city in cities){ # loop through distances
  ...
  print(kable(df, caption = paste("Model age population distance by mode per day for  ", city)))
}
...
City specific duration tables for baseline and scenarios (per day in minutes)


```{r trip_dur = "asis"}
# create tables showing the average duration each person spends travelling per mode 
...
for(city in cities){ # loop through cities
  ...
  # print to html
  print(kable(td, caption = paste("Duration table (mins) per day for model synthetic population for ", city, "( ", count_people, " ) per person (everyone)")))
...
# find injury death column
#injury_col <- which(colnames(io[[1]]$outcomes$hb$deaths) == paste0(scen_names[2],'_deaths_inj'))

# find ap death columns
...
Change in total deaths compared to the reference scenario (for the city based on real population size) by scenario
...
Change in deaths per 100,000 people by scenario in comparison to the reference scenario
...
Change in total YLLs by disease compared to the reference scenario (for the city based on real population size, for ages considered within the ITHIM model) 
  ...
  for (j in pathway) { # loop for hb or pathway_hb
    for (i in names(io[-2])) { # loop for cities
      ...
      health[[k]][[j]][[i]] <- gather(temp, key = "cause", "measure", 
        ...
        mutate(metric_100k = measure / pop_age * 100000, 
               metric_100k_sex = measure / pop_age_sex * 100000)
