...
knitr::opts_chunk$set(comment=NA, prompt=FALSE, cache=FALSE, echo=F, results='asis', warning = F)
...
dat[[city]] <- io[[city]]$trip_scen_sets %>% mutate(scenario = case_when(
  scenario == "Scenario 1" ~ "PED_SC", 
  scenario == "Scenario 2" ~ "CYC_SC",
  scenario == "Scenario 3" ~ "CAR_SC",
  scenario == "Scenario 4" ~ "MOT_SC",
  scenario == "Scenario 5" ~ "BUS_SC",
  scenario == "Baseline" ~ "Baseline")) %>% group_by(scenario, trip_mode, trip_distance_cat) %>% summarise(n = dplyr::n(), dist = sum(trip_distance, na.rm = T)) %>% mutate(city = city)
...
for(city in cities){
  ...
  colnames(td)[2:7] <- c("BASELINE", rownames(scen_prop))
...
Case study specific duration tables for baseline and five scenarios (per day in minutes)
...
# Export ylls HB
write.csv(rbindlist(health$ylls$hb, use.names = T),
            "results/multi_city/health_impacts/ylls_hb_lng.csv", 
            row.names = F)
# Export ylls PATHWAY_HB
write.csv(rbindlist(health$ylls$pathway_hb, use.names = T),
            "results/multi_city/health_impacts/ylls_pathway_hb_lng.csv", 
            row.names = F)
# Export deaths HB
write.csv(rbindlist(health$deaths$hb, use.names = T),
            "results/multi_city/health_impacts/deaths_hb_lng.csv", 
            row.names = F)
# Export deaths PATHWAY_HB
write.csv(rbindlist(health$deaths$pathway_hb, use.names = T),
            "results/multi_city/health_impacts/deaths_pathway_hb_lng.csv", 
            row.names = F)
