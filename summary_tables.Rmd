...
pa_totals <- lapply(io[-2],function(x){
  ...
  xxx <- sapply(1:NSCEN, function(y){
    xxx <- rowSums(xx[, pa_cols[seq(y, length(pa_cols), by = NSCEN)]])
...
ap_totals <- lapply(io[-2],function(x){
  xx <- x$outcomes$pathway_hb$ylls
  xxx <- sapply(1:NSCEN,function(y) {
    xxx <- rowSums(xx[, ap_cols[seq(y, length(ap_cols), by = NSCEN)]])
    sapply(sort(unique(xx$age_cat)), function(z) sum(xxx[xx$age_cat == z]))
  })
  colnames(xxx) <- scen_names
  xxx
...
writexl::write_xlsx(list(ylls_hb_lng = rbindlist(health$ylls$hb, use.names = T),
                         ylls_pathway_hb_lng = rbindlist(health$ylls$pathway_hb, use.names = T),
                         Deaths = rbindlist(health$deaths$hb, use.names = T),
                         Deaths_Pathway = rbindlist(health$deaths$pathway_hb, use.names = T)), path = output_file)
