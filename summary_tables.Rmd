...
# Script summarises mode splits, distances and durations travelled and looks at health outcomes for the different air pollution, injury and physical activity pathways but also by disease
...
## Distances travelled
# - city specific distance tables showing average daily distances in km travelled by each person in the travel survey by mode for all scenarios (incl Baseline)

# - city specific distance tables showing average daily distances in km travelled by each person in the travel survey WITH a trip by mode for all scenarios (incl Baseline)

# - one table giving the city specific baseline average daily distances (km) travelled by each person in the respective travel surveys for all cities

# - city specific distance tables showing total yearly distances in km travelled by the total  population considered in the model for each mode for all scenarios (incl Baseline)

# - one table giving the city specific total yearly baseline distances in km travelled by the total population for each mode for all cities

# - city specific distance tables showing total daily distances in km travelled by the total population for each mode for all scenarios (incl Baseline)
...
## Travel duration
# - city specific duration tables showing average daily duration (h) spent travelling by each person in the travel survey by mode for all scenarios (incl Baseline)

# - city specific duration tables showing average daily duration (h) spent travelling by each person in the travel survey WITH a trip by mode for all scenarios (incl Baseline)

# - one table giving the city specific baseline average duration (h) spent travelling by each person in the respective travel surveys for all cities
...
# - the ylls.csv file (results/multi_city/health_impacts/ylls.csv) gives the changes in YLLs for each age and sex category, for each age category per 100,000 people, and for each age and sex category per 100,000 people, for each city, disease, scenario, age and sex category. It also shows the disease level, whether the disease is exasperated by changes in AP, PA or both, and, the age and age and sex specific population levels. 



# - the ylls_pathway.csv file (results/multi_city/health_impacts/ylls_pathway.csv)
...
# - the deaths_pathway.csv file (results/multi_city/health_impacts/deaths_pathway.csv)
...
# - set up helper function to re-format large numbers and re-name scenarios

# ???????????????

# call get_scen_settings.R function to find the Baseline mode splits for each distance category for each city 
...
Case study specific trip proportions by mode, for baseline and scenarios
...
# find the modal share of for all cities and scenarios
