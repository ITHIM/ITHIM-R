...
### Baseline average distance (km) for all case studies
```{r}

kable(trip_dist, caption = "Avg. distance per person (km) across all case studies")
...
for(city in cities){
  ...
  td <- io[[city]]$true_dist %>% filter(stage_mode != 'bus_driver') %>% mutate_if(is.numeric, list(~round((.) * 52, 3)))
  colnames(td)[3:7] <- rownames(scen_prop)
...
## Baseline total distance tables for all case studies (per year in km)
...
## True Distance by total population
### Tables
...
for (city in cities){
  ...
  print(kable(df, caption = paste("True distance by mode for  ", city)))
}
