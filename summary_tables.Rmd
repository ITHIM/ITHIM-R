...
dat[[city]] <- io[[city]]$trip_scen_sets %>% mutate(scenario = case_when(
  ...
  scenario == "Baseline" ~ "Baseline")) %>% group_by(scenario, trip_mode, trip_distance_cat) %>% summarise(n = dplyr::n()) %>% mutate(city = city) %>% filter(!(trip_mode %in% "truck"))
...
for (i in 1:nrow(td)){
    ...
    if (nrow(val) > 0){
      ...
      td$prop[i] <- round(td$n[i] / val$n, 2)
