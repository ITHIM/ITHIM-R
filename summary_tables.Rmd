...
scen_names <- rownames(io$scen_prop)
  ...
  for (j in pathway) { # loop for hb or pathway_hb
    for (i in names(io[-2])) { # loop for cities
      ...
      health[[k]][[j]][[i]] <- gather(temp, key = "cause", "measure", 
        ...
        mutate(city = i,
               scenario = case_when(
                 grepl("sc_cycle", cause) ~ scen_names[1],
                 grepl("sc_car", cause) ~ scen_names[2],
                 grepl("sc_bus", cause) ~ scen_names[3],
                 grepl("sc_motorcycle", cause) ~ scen_names[4]
               ),
               level1 = case_when(
...
for (city in cities){
  ...
  td <- io[[city]]$outcomes$mmets %>% dplyr::select(ends_with("mmet")) |> 
    rename_with(recode, base_mmet = "Baseline", sc_cycle_mmet = "CYC_SC",
           sc_car_mmet = "CAR_SC",  sc_bus_mmet = "BUS_SC",
           sc_motorcycle_mmet = "MOT_SC") %>% 
