...
global_path <- file.path(find.package('ithimr',lib.loc = .libPaths()), 
                         'extdata/global/')
...
for (j in c(1:nrow(DISEASE_INVENTORY))[DISEASE_INVENTORY$air_pollution == 1]) { 
  ...
  for (age in ages) {
    ...
    alpha <- sapply(DR_AP_LIST,function(x)x[[cause]][[age]]$alpha)
    beta <- sapply(DR_AP_LIST,function(x)x[[cause]][[age]]$beta)
    gamma <- sapply(DR_AP_LIST,function(x)x[[cause]][[age]]$gamma)
    tmrel <- sapply(DR_AP_LIST,function(x)x[[cause]][[age]]$tmrel)
