...
```{r genTotalHistPlots, message = T, echo = F, warning = F}

rd1 <- io$accra$trip_scen_sets %>% mutate(cs = 'Accra') %>% select(cs, trip_distance, trip_distance_cat)
rd2 <- io$sao_paulo$trip_scen_sets %>% mutate(cs = 'Sao Paulo') %>% select(cs, trip_distance, trip_distance_cat)
rd3 <- io$delhi$trip_scen_sets %>% mutate(cs = 'Delhi') %>% select(cs, trip_distance, trip_distance_cat)

rd <- plyr::rbind.fill(rd1, rd2, rd3)

op <- ggplot(rd %>% filter(trip_distance < 51)) +
  aes(x = trip_distance, fill = cs) +
  geom_histogram(breaks = c(0:10,seq(11, 51, by = 5)), alpha=0.8, position="identity", aes(y = ..density..)) +
  labs(x = '') + 
