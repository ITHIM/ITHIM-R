...
raw_rd <- read_csv("data/local/buenos_aires/buenos_aires_trip.csv")
...
if (nrow(rd[rd$trip_mode %in% c("bus", "train") & rd$stage_mode == "walk",]) > 0)
  rd$stage_mode[rd$trip_mode %in% c("bus", "train", 'metro') && rd$stage_mode == "walk"] <- "walk_to_pt"
...
#####
# Write csv
write_csv(rd, "inst/extdata/local/buenos_aires/trips_buenos_aires.csv")
