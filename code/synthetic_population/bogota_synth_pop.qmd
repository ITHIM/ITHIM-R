...
# Compare Travel Survey distribution with and without individual weights

Daniel has kindly added `weights` to each row in the Travel Survey dataset, which represents individual weights. In this section we compare distribution based on `trip_mode`, `stage_mode` and `sex` distribution with and without the individual weights. In all the tables below, the column `raw_trip_freq` shows distribution [**without**]{.underline} and `weighted_trip_freq` [**with**]{.underline} `weights`.
