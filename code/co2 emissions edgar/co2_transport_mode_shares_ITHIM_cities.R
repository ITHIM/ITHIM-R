...
path <- 'code/co2 emissions edgar/'


##reading CO2 files specific to each mode of transport
bus <- read.table(paste0(path, 'txt_TRO_TNR/monica_v5.0_EM_CO2_TRO_ROA_BS0_2012_CO2_2012.txt'),
                         header = T, sep = ";")
bus$mode <- "bus"
all_modes <- bus

htrucks <- read.table(paste0(path, 'txt_TRO_TNR/monica_v5.0_EM_CO2_TRO_ROA_HD0_2012_CO2_2012.txt'), 
                     header = T, sep = ";")
htrucks$mode <- "htrucks"
all_modes <- rbind(all_modes, htrucks)

lttrucks <- read.table(paste0(path, 'txt_TRO_TNR/monica_v5.0_EM_CO2_TRO_ROA_LD0_2012_CO2_2012.txt'), 
                       header = T, sep = ";")
lttrucks$mode <- "lttrucks"
all_modes <- rbind(all_modes, lttrucks)

mtw <- read.table(paste0(path, 'txt_TRO_TNR/monica_v5.0_EM_CO2_TRO_ROA_MC0_MP0_2012_CO2_2012.txt'), 
                  header = T, sep = ";")
mtw$mode <- "mtw"
all_modes <- rbind(all_modes, mtw)

car <- read.table(paste0(path, 'txt_TRO_TNR/monica_v5.0_EM_CO2_TRO_ROA_PC0_2012_CO2_2012.txt'), 
                  header = T, sep = ";")
car$mode <- "car"
all_modes <- rbind(all_modes, car)

other <- read.table(paste0(path, 'txt_TRO_TNR/monica_v5.0_EM_CO2_TNR_OTH_2012_CO2_2012.txt'), 
                    header = T, sep = ";")
other$mode <- "other"
...
select <- rgdal::readOGR('code/co2 emissions edgar/cities.shp')
...
emissions <- co2 %>% group_by(Name, mode) %>% 
  summarise("total" = sum(co2emissions))
city_emissions <- spread(emissions, "mode", "total" )
# --- Daniel
# This is the object that I used for latam cities
...
lookup<- read.csv('code/co2 emissions edgar/lat_long_ithim_cities_edgar_emissions_lookup.csv')
