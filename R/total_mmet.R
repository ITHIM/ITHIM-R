...
total_mmet <- function(trip_scen_sets){
  ...
  rd_pa <- subset(trip_scen_sets,trip_mode%in%c('walking','walk_to_bus','bicycle')&participant_id>0) 
  ...
  for (i in 1:length(SCEN)){
    ...
    individual_data <- setDT(scen_trips)[,.(cycling_mmet_base = sum(trip_duration_hrs[trip_mode == 'bicycle']) * MMET_CYCLING,
                                            walking_mmet_base = sum(trip_duration_hrs[trip_mode %in%c('walking','walk_to_bus')]) * MMET_WALKING ),by='participant_id']
