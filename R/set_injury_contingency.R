...
set_injury_contingency <- function(injuries){
  ...
  ##!! need to work out of logic of how we know which modes there are distances for!
  mode_names <- intersect(unique(TRIP_SET$stage_mode),MODE_SPEEDS$stage_mode)
  # If pedestrian mode is missing, then add it
  if (!any(mode_names %in% 'pedestrian'))
    mode_names <- c(mode_names,"pedestrian")
  mode_names <- mode_names[mode_names!='other']
  if(ADD_BUS_DRIVERS && !any(mode_names %in% 'bus_driver'))
    mode_names <- c(mode_names,'bus_driver')
  if(ADD_TRUCK_DRIVERS && !any(mode_names %in% 'truck'))
    mode_names <- c(mode_names,'truck')
  if(CITY=='accra' && !any(mode_names %in% 'motorcycle'))
    mode_names <- c(mode_names,'motorcycle')
  
  mode_names <- unique(mode_names)
  injury_list <- list()
