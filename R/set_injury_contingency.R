...
set_injury_contingency <- function(injuries){
  ...
  mode_names <- c(intersect(unique(TRIP_SET$stage_mode),MODE_SPEEDS$stage_mode),"pedestrian")
  mode_names <- mode_names[mode_names!='other']
  if(ADD_BUS_DRIVERS && !mode_names %in% 'bus_driver') mode_names <- c(mode_names,'bus_driver')
  if(ADD_TRUCK_DRIVERS && !mode_names %in% 'truck') mode_names <- c(mode_names,'truck')
  if(CITY=='accra' && !mode_names %in% 'motorcycle') mode_names <- c(mode_names,'motorcycle')
  injury_list <- list()
