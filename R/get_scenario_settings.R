...
get_scenario_settings <- function(cities = c('accra', 'bangalore', 'belo_horizonte', 
  ...
  for(j in 1:length(mode_proportions_distance_list)){
    {
      cat("\n")
      cat(  paste0('|',names(mode_proportions_distance_list)[j],'|',(paste0(colnames(mode_proportions_distance_list[[j]]),
                                                                            collapse='|')),'|\n|---|---|---|---|\n'))
      for(i in 1:nrow(mode_proportions_distance_list[[j]])) cat('|',rownames(mode_proportions_distance_list[[j]])[i],'|',
                                                                paste0(sapply(mode_proportions_distance_list[[j]][i,],
                                                                              function(x)sprintf('%.1f',x)),collapse='|'),'|\n')
    }
  }
