...
create_cycle_scenarios <- function(trip_set){
  ...
  # Baseline scenario
  rd_list[[1]] <- rdr
  print(c(sum(rdr$trip_distance_cat==target_distance),sum(rdr$trip_distance_cat==target_distance&rdr$trip_mode=='bike')))
  ###############################################################
  for(i in 1:5){
    # Scenario i: i*10% cycle
    short_trips <- subset(rdr,trip_distance_cat==target_distance)
    potential_trip_ids <- unique(subset(short_trips,trip_mode!='bike')$trip_id)
    current_cycle_trips <- sum(short_trips$trip_mode=='bike')
    target_percent <- 10*i
    ...
    change_trips$trip_mode <- 'bike'
    change_trips$stage_mode <- 'bike'
    change_trips$stage_duration <- change_trips$stage_distance * 60 / MODE_SPEEDS$speed[MODE_SPEEDS$stage_mode=='bike']
