...
for (x in 1: length(cities))
{
  ...
  x<-ggplot(data_long, aes(x=scenario, y=pm_conc, fill=scenario)) + geom_boxplot(outlier.shape = NA)+ ggtitle(cities[x])+ labs(y="Daily PM2.5 Concentration", x = "Scenarios")+ylim(low[x],high[x])
...
for (x in 1: length(cities))
{
  ...
  x<-ggplot(data_long, aes(x=scenario, y=mmet, fill=scenario)) + geom_boxplot(outlier.shape = NA)+ ggtitle(cities[x])+ labs(y="Weekly MMET", x = "Scenarios")+  ylim(0, limit)
  print(x)
