...
# Script to summarise the PM2.5 exposure and levels, PM2.5 and CO2 concentrations and a physical activity levels
...
# WARNING: Script assumes makes all comparisons to the Baseline (and not the reference) scenario
...
# - one table for each city detailing the individual PM2.5 exposure levels incl the mean, 5th, 50th and 95th percentiles, the mean PM2.5 concentration levels for each scenario and the change in PM2.5 compared to the baseline scenario
...
# - one table detailing the CO2 concentration levels for each scenario for all cities
...
# - one table for each city detailing the individual PM2.5 exposure levels incl the mean, 5th, 50th and 95th percentiles, the mean PM2.5 concentration levels for each scenario and the change in PM2.5 compared to the baseline scenario. Table also contains CO2 concentrations for each scenario
...
#   - summary_stats_PM2.5_CO2: summary statistics of individual PM2.5 exposure levels, the total PM2.5 concentrations levels, the change in PM2.5 concentration levels compared to the baseline scenario and the total transport related CO2 emission concentrations for each scenario and city
...
#### The script performs the following steps, assuming that ITHIM-GLOBAL module has been run and the ithim_objects object has been saved in "results/multi_city/io.rds":
...
# - create one table for each city detailing the individual PM2.5 exposure levels incl. the mean, 5th, 50th and 95th percentiles, the mean PM2.5 concentration levels for each scenario and the change in PM2.5 compared to the baseline scenario
...
# - create one table for each city detailing the individual PM2.5 exposure levels incl the mean, 5th, 50th and 95th percentiles, the mean PM2.5 concentration levels for each scenario and the change in PM2.5 compared to the baseline scenario. Table also contains CO2 concentrations for each scenario
...
reference_scenario <- io[[city]]$ithim_run$reference_scenario

compute_mode <- io[[city]]$ithim_run$compute_mode # constant parameters from the given parameters
...
# for each city create a summary table of individual PM2.5 exposure levels giving the mean and the 5th, 50th and 95th percentile levels. Tables also show the PM2.5 concentration levels in the city for each scenario and the changes compared to the baseline scenario
...
for (x in 1:length(cities)) {
  ...
  # add scenario specific PM2.5 concentration levels and the change in PM2.5 compared to the baseline
...
# Average physical activity levels (MMETs) in each city
...
for (x in 1:length(cities)) { # loop through cities
  ...
  # calculate difference to baseline scenario
