  ...
  for (pref in (word(grep("whw",names(io[[city]]$outcomes$whw$Baseline), value = T), 2, sep = "_"))){
    ...
    for (cs in names(io[[city]]$outcomes$whw)){
      ...
      if (is.null(io[[city]]$outcomes$whw[[cs]][[whw_qn]])){
        if (!is.null(io[[city]]$outcomes$whw[[cs]][[nov_qn]])){
        ...
        td1 <- (io[[city]]$outcomes$whw[[cs]][[whw_qn]]) %>% as.data.frame() %>% tibble::rownames_to_column("mode")
        td3 <- td1
        if (!is.null(io[[city]]$outcomes$whw[[cs]][[nov_qn]])){
          td2 <- t(io[[city]]$outcomes$whw[[cs]][[nov_qn]]) %>% as.data.frame()
          td2$mode <- 'NOV'
      ...
      city_df[[var]][[city]][[unique(td$measure)]] <- td
      
      if (length(city_df[[var]][[city]]) == 3)
        all_whw[[var]][[city]] <- data.table::rbindlist(city_df[[var]][[city]])
    }
