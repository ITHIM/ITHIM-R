...
# NOTE: the following scenario order is assumed: 'bi_sc', 'car_sc', 'bus_sc','mc_sc','w_sc' and if there are less than 5 scenarios, the last scenarios can be missing but all of the scenarios in the middle need to exist. e.g. if you want the public transport and the walking scenario, you still need to create a car, motorcycling and bicycling scenario!
...
scen_names_w_baseline <- c('bl',  'bi_sc', 'car_sc', 'bus_sc','mc_sc','w_sc')
if (NSCEN == 4){
  scen_names_w_baseline <- c('bl', 'bi_sc', 'car_sc', 'bus_sc', 'mc_sc')
} else if (NSCEN == 3){
  scen_names_w_baseline <- c('bl', 'bi_sc', 'car_sc', 'bus_sc')
}
...
true_distances[true_distances$scenario == 'Scenario 5',]$scenario <- 'Walking'

# add mode distances
whw_list <- left_join(whw_list, true_distances, by = c('city', 'cas_mode','scenario'))

# combine active travel modes
active_travel <- whw_list %>% filter(cas_mode == 'cycle' | cas_mode == 'pedestrian')
...
for (city in cities){
  ...
  for (pref in (word(grep("whw",names(io[[city]]$outcomes$whw$Baseline), value = T), 2, sep = "_"))){
    ...
    for (cs in names(io[[city]]$outcomes$whw)){
      ...
      td5 <- td4 # save copy for risk per population
...
if (NSCEN == 4){
  injury_risks_lng_hours[injury_risks_lng_hours$scenario == 'mcsc',]$scenario <- 'Motorcycling'
}
if (NSCEN == 5){
  injury_risks_lng_hours[injury_risks_lng_hours$scenario == 'mcsc',]$scenario <- 'Motorcycling'
    injury_risks_lng_hours[injury_risks_lng_hours$scenario == 'wsc',]$scenario <- 'Walking'
}
...
duration[duration$scenario == 'Scenario 5',]$scenario <- 'Walking'
