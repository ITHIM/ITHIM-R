...
for (city in cities){
  ...
  for (cs in names(io[[city]]$outcomes$whw)){
    if (length(names(io[[city]]$outcomes$whw$Baseline)) == 2){
      ...
      td1 <- io[[city]]$outcomes$whw[[cs]]$nov %>% as.data.frame() %>% rownames_to_column() %>% rename(mode = rowname) %>% rename_at(2, ~"count")
      # td2: Number of fatalities in whw
      td2 <- colSums(io[[city]]$outcomes$whw[[cs]]$whw) %>% as.data.frame() %>% rownames_to_column() %>% rename(mode = rowname) %>% rename_at(2, ~"count")
      ...
      td3 <- colSums(io[[city]]$outcomes$whw[[cs]]$whw) %>% as.data.frame() %>% rownames_to_column() %>% rename(mode = rowname) %>% rename_at(2, ~"count")
      #td3[td3$mode == 'pedestrian', ]$mode <- 'walking'
    }else if(length(names(io[[city]]$outcomes$whw$Baseline)) == 1 && names(io[[city]]$outcomes$whw$Baseline) == 'nov'){
      
      td3 <- io[[city]]$outcomes$whw[[cs]]$nov %>% as.data.frame() %>% rownames_to_column() %>% rename(mode = rowname) %>% rename_at(2, ~"count")
      #td3[td3$mode == 'pedestrian', ]$mode <- 'walking'
...
for (city in cities) { # Loop for each city
  true_distances[[city]] <- io[[city]]$true_dist %>% 
    pivot_longer(!stage_mode, names_to = "scenario", values_to = "distance") %>% 
    mutate(city = city)
}
true_distances_df <- bind_rows(true_distances)
