...
Creates three scenarios where in each one, the mode share of a given
mode is elevated by a set percentage of the total trips. The scenario
modes are cycle, car, and bus.
...
- trip_set:
  ...
  data frame, baseline scenario trips
...
This function creates three scenarios increasing the mode shares of
cycling, car and bus by the same pre-defined percentage of the total
mode shares. We assume that:
...
- the percentage share across the three distance bands of the mode that
  ...
  the cycling mode share by x we still have 10
...
- we preserve the proportion of trips in each distance band. E.g. if 20
  ...
  are still in distance band 0-2km.
...
- the overall mode shares for each of the cycle, car and bus modes
  across the three distance categories is extracted from the trip data
  set
