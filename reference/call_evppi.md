...
evppi_df containing the evppi values for all input parameters and
scenario and disease outcomes

evppi_outcome_names, a vector containing all the scenario outcome
combinations
...
- loop through the cities:
  ...
  - extract the RR values for each dose response function and outcome,
    calculate the RR for the different levels and for the entire
    population

  - extract the city specific parameters (excluding the CO2 and PM
    emission inventory parameters as they are not independent of each
    other)
