...
sex <- c("male", "female")
cause <- c("all causes", "ischemic heart disease", "ischemic stroke", "diabetes mellitus", "colon and rectum cancers", "breast cancer", "tracheal, bronchus, and lung cancer")
measure <- c("all causes", "ylds (years lived with disability)", "prevalence", "incidence")


for (i in sex){
  for (j in cause) {
    for (k in measure) {
      p <- paste("Plot", "compare", sex, cause, measure, sep = "_")
      assign(p, plot_GBD(in_data1 = GBDEngland, in_data2 = GBDGL, in_sex = sex, in_cause = cause, in_measure = measure))
      print(p)
    }
  }
}  
...
```{r, echo=FALSE}
#Call plots here, add titles to figures
