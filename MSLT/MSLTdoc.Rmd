...
6) Create age categories index in GBDdata (upper limit age group to match injury data), total of 20 age groups. These are the age cohorts to simulate. 
```{r, echo=TRUE}
GBDdata$age_cat [GBDdata$age =="under 5"] <- 5
GBDdata$age_cat [GBDdata$age =="5 to 9"] <- 10
GBDdata$age_cat [GBDdata$age =="10 to 14"] <- 15
GBDdata$age_cat [GBDdata$age =="15 to 19"] <- 20
GBDdata$age_cat [GBDdata$age =="20 to 24"] <- 25
GBDdata$age_cat [GBDdata$age =="25 to 29"] <- 30
GBDdata$age_cat [GBDdata$age =="30 to 34"] <- 35
GBDdata$age_cat [GBDdata$age =="35 to 39"] <- 40
GBDdata$age_cat [GBDdata$age =="40 to 44"] <- 45
GBDdata$age_cat [GBDdata$age =="45 to 49"] <- 50
GBDdata$age_cat [GBDdata$age =="50 to 54"] <- 55
GBDdata$age_cat [GBDdata$age =="55 to 59"] <- 60
GBDdata$age_cat [GBDdata$age =="60 to 64"] <- 65
GBDdata$age_cat [GBDdata$age =="65 to 69"] <- 70
GBDdata$age_cat [GBDdata$age =="70 to 74"] <- 75
GBDdata$age_cat [GBDdata$age =="75 to 79"] <- 80
GBDdata$age_cat [GBDdata$age =="80 to 84"] <- 85
GBDdata$age_cat [GBDdata$age =="85 to 89"] <- 90
GBDdata$age_cat [GBDdata$age =="90 to 94"] <- 95
GBDdata$age_cat [GBDdata$age =="95 plus"] <- 100
```
...
```{r, echo=TRUE}
GBD_population <- filter(GBDdata, measure == "deaths", cause == "all causes", metric == "rate" | metric == "number" ) %>% select(metric, age_cat, sex, val, sex_age_cat, location)
```
...
```{r, echo=TRUE}
 for (i in 1:nrow(GBD_population)) {
    if (GBD_population$metric[i] == "number") {
      GBD_population$pop_five_year[i] <- GBD_population$val[i] * 100000/ GBD_population$val[i + 2]}
   else {GBD_population$pop_five_year[i] <- NA}
 }
```
11) Remove rows with zero
```{r, echo=TRUE}
GBD_population <- GBD_population[!is.na(GBD_population$pop_five_year),]
```
12) Keep relevant variables
```{r, echo=TRUE}
GBD_population <- filter(GBD_population) %>% select(sex_age_cat, sex, age_cat, pop_five_year, location)
```
13) Create data frames for Greater London to be later used for: a) interpolation of rates and b) PMLT cohorts. 
```{r, echo=TRUE}
GBD_population_GL <-  filter(GBD_population, location == "greater london") %>% select(sex_age_cat, age_cat, sex, pop_five_year, location)
```
12) Check population total numbers
```{r, echo=TRUE}
GreaterLondon <- sum(GBD_population_GL$pop_five_year)
```
...
data <- filter(GBDGL, measure == measure_index, sex == sex_index, cause == "all causes") %>% select(measure, location, sex, age, metric, cause, val, age_cat, one_rate) 
...
interpolated <- as.data.frame(interpolation_func(seq(0, 99, 1)))
ages <- seq(0,99,by=1)
interpolated <- cbind(interpolated, ages)
interpolated[,1] <- exp(interpolated[,1])
colnames(interpolated)[1] <- "rate"
...
p <- ggplot(data = interpolated,mapping = aes(ages,rate)) +
              geom_line(aes(color = "Interpolated")) +
              geom_point(
              data = data, 
              mapping = aes(age_cat, one_rate, color = "Original")) +
              labs(colour="",x="Age",y= paste(i_sex, i_measure, "rates", sep = " "))+
            labs (title = paste("Rates", i_sex, i_measure, sep = " "), size=14) +
            theme_classic() +
...
```{r, echo=TRUE}
##Create life table input data frame (population numbers, all-cause moratlity rates ##(per one) and all cause yld rates)

##Create empty data frame
input_life_table <- as.data.frame(matrix(0, ncol=4, nrow = 202))
##Give variables names (mx=total mortality rate, yldsx = total years lived with disability rates )
names(input_life_table) <- c("age", "sex", "mx", "yldsx")
...
input_life_table$sex_age_cat <- paste(input_life_table$sex,input_life_table$age, sep = "_"  )


##Populate life table: five_year_pop. SEE LIFE TABLE LOOPS THAT USES 5 YEAR POP. 

input_life_table <- merge(input_life_table, select(GBD_population_GL
                                                     ,c(sex_age_cat, pop_five_year))
                                                     , by = 'sex_age_cat', all.x = TRUE) 

#input_life_table$pop_five_year <- ifelse(
#  input_life_table$sex_age_cat == GBD_population_GL$pop_five_year, #GBD_population_GL$pop_five_year, NA)
