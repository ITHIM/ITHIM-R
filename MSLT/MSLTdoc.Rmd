...
```{r, echo=TRUE}
GBD_population <- filter(GBD_population) %>% select(sex_age_cat, sex, age_cat, five_year_population, location)
```
13) Create data frames for Greater London to be later used for: a) interpolation of rates and b) PMLT cohorts. 
```{r, echo=TRUE}
GBD_population_GL <-  filter(GBD_population, location == "greater london") %>% select(sex_age_cat, age_cat, sex, five_year_population, location)
```
...
# Change name of column death to mx and ylds to pyld_rate to then merge with input_life table
...
```{r, echo=TRUE}
##Create life table input data frame (population numbers, all-cause moratlity rates ##(per one), all cause yld rates) and 5-year age groups 
## to run the cohorts (from population life table). 
...
## I created mx.x and mx.y and pyld.x and pyld.y and I only need mx and pyld_rate in one colument. I wil do a quick fix here. 
## DISCUSS WITH ALI TO IMPROVE

input_life_table$mx <-  ifelse(input_life_table$sex == "male", input_life_table$mx.x, input_life_table$mx.y)
input_life_table$pyld_rate <-  ifelse(input_life_table$sex == "male", input_life_table$pyld_rate.x, input_life_table$pyld_rate.y)
...
## The function run_life_table was created to run the basline and scenario life tables (see functions.R script). 
## The input_life_table data frame was generated to work with the function. 

## Generate looping variables (sex and age_group. Sex already generated in code for interpolation (i_sex))
...
for (age in i_age_cohort){
  for (sex in i_sex){
    ...
    general_life_table_list_bl[[index]] <- run_life_table(in_idata = input_life_table, in_sex = sex, in_mid_age = age)
    index <- index + 1
  }
}
...
17) Generate disease life tables. 
Generate two sets for Greater London: 1) GBD data and 2) English survey data. 
