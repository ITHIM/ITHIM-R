...
data <- filter(GBDGL, measure == "ylds (years lived with disability)", sex == "female", cause == "all causes") %>% select(measure, location, sex, age, metric, cause, val, age_cat, one_rate)

x=data$age_cat
y=log(data$one_rate)

interpolated<- as.data.frame(spline(x, y, n = 5 *length (x), xmin = min(x-2), xmax = max(x + 2)))
interpolated$y <- exp(y)
plot(x,y)
...
for(sex in i_sex) {
  for (measure in i_measure) {
    ...
    #print(paste(sex, measure, "interpolated_data", sep = "_"))
    #print(paste(sex, measure, "interpolated", sep = "_"))
...
plot(male_deaths_interpolated_data$age_cat, male_deaths_interpolated_data$one_rate)

plot(female_deaths_interpolated$x, female_deaths_interpolated$y)
