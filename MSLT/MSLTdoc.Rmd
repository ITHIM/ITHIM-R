...
for(sex in i_sex) {
  for (measure in i_measure) {
    ...
    data <- filter(GBDGL, measure == i_measure, sex == i_sex, cause == "all causes") %>% select(measure, location, sex, age, metric, cause, val, age_cat, one_rate) 
    assign(paste(i_sex, i_measure, "interpolated_data", sep = " "), data)
x=data$age_cat
y=log(data$one_rate)

interpolated <- as.data.frame(spline(x, y, n = 5 *length (x), xmin = min(x-2), xmax = max(x + 2)))
interpolated$y <- exp(y)
assign(paste(i_sex, i_measure, "interpolated", sep = " "), interpolated)
plot(x,y)
