...
```{r, echo=FALSE}
## THIS IS AN ERROR WHEN TRYING TO KNIT THE DOC, PROBLEM WITH THE FILE PATH. 
...
# 
# myimages <- list.files("graphs2/", pattern = ".jpeg", full.names = TRUE)
# include_graphics(myimages)

#for (i in 1:64){
#  knitr::include_graphics(graphs_doc[[i]]) 
#}
```
...
## First, need to run function for males and females separetly, in_cohorts indicates the number of age cohorts to include. To show all select 16. what is specified in in_outcomes will be graphed or can also be presented as a total. 

## DO A LIST TO GENERATE ALL OUTCOMES ALL. CHECK SUMMARION OF COHORTS ORDER. 
## Generate vectors with names of outcomes of interest: baseline, scenario and difference (from output dataframe)

i_output_bl <- c("inc_num_bl_ihd", "inc_num_bl_istroke", "inc_num_bl_diabetes", "inc_num_bl_colon_cancer", "inc_num_bl_breast_cancer", "mx_num_bl_ihd", "mx_num_bl_istroke", "mx_num_bl_diabetes", "mx_num_bl_colon_cancer", "mx_num_bl_breast_cancer", "Lx_bl", "Lwx_bl")

i_output_sc <- c("inc_num_sc_ihd", "inc_num_sc_istroke", "inc_num_sc_diabetes", "inc_num_sc_colon_cancer", "inc_num_sc_breast_cancer", "mx_num_sc_ihd", "mx_num_sc_istroke", "mx_num_sc_diabetes", "mx_num_sc_colon_cancer", "mx_num_sc_breast_cancer", "Lx_sc", "Lwx_sc")

i_output_diff <- c("inc_num_diff_ihd", "inc_num_diff_istroke", "inc_num_diff_diabetes", "inc_num_diff_colon_cancer", "inc_num_diff_breast_cancer", "mx_num_diff_ihd", "mx_num_diff_istroke", "mx_num_diff_diabetes", "mx_num_diff_colon_cancer", "mx_num_diff_breast_cancer", "Lx_diff", "Lwx_diff")

## SEPARATE LOOPS FOR MALES AND FEMALES
aggregate_frame_males <- list()
aggregate_frame_females <- list()
index <- 1

for (output_bl in i_output_bl) {
  for (output_sc in i_output_sc) {
    for (output_diff in i_output_diff) {
      
aggregate_frame_males[[index]]<- gen_aggregate(in_data = output_df, in_cohorts = 16, in_population = "male", in_outcomes = c(paste(output_bl), paste(output_sc), paste(output_diff)))
      
# aggregate_frame_males <- gen_aggregate(in_data = output_df, in_cohorts = 16, in_population = "male", in_outcomes = c(output_bl, output_sc, output_diff))

aggregate_frame_females[[index]] <- gen_aggregate(in_data = output_df, in_cohorts = 16, in_population = "female", in_outcomes = c(paste(output_bl), paste(output_sc), paste(output_diff)))

# aggregate_frame_females <- gen_aggregate(in_data = output_df, in_cohorts = 16, in_population = "female", in_outcomes = c(output_bl, output_sc, output_diff))
...
# 
# ## Uncomment to view aggregated data
# sum(total_aggr$total_Lx_diff)
```
