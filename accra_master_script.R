...
for(mc in c(1,2)){
  ...
  distances <- mclapply(1:NSAMPLES,just_distances,ithim_object=ithim_object,mc.cores=4)
  #distances <- list()
  #for(i in 1:NSAMPLES) distances[[i]] <- just_distances(i,ithim_object)
  #}
  SCEN <- 'walk'
  scen_names <- c('baseline',SCEN)
  NSCEN <- length(SCEN)
  #for(city in cities){
  dist_mat <- matrix(0,nrow=NSAMPLES,ncol=nrow(distances[[1]]$dist))
  pdf(paste0('distance_distribution_1000p_',mc,'.pdf'),width=10,height=5)
  #x11(); 
  par(mfrow=c(1,2),mar=c(7,5,2,1))
  for(j in 1:2){
    for(i in 1:NSAMPLES)
      dist_mat[i,] <- distances[[i]]$dist[,j]/nrow(distances[[i]]$pp_summary[[1]])
    boxplot(dist_mat,names=rownames(distances[[i]]$dist),las=2,frame=F,main=scen_names[j],ylab='km pp')
  }
  dev.off()
}
...
ithim_object <- run_ithim_setup(NSAMPLES = 1024,
                                ...
                                DISTANCE_SCALAR_CYCLING = c(0,log(1.2)),
                                DISTANCE_SCALAR_PT = c(0,log(1.2)),
                                PROBABILITY_SCALAR_MOTORCYCLE = 2,
...
propensity_names <- names(ithim_object$parameters$PROPENSITY_TO_TRAVEL[[1]])
propensity_parameters <- as.data.frame(t(sapply(ithim_object$parameters$PROPENSITY_TO_TRAVEL,function(x)unlist(x))))
colnames(propensity_parameters) <- paste0('PROPENSITY_TO_TRAVEL_',propensity_names)
parameter_samples <- cbind(parameter_samples,propensity_parameters)
...
{
  pdf('accra_walk_yll_uncertain_travel.pdf'); par(mar=c(5,5,2,2))
...
print(evppi)
{
pdf('accra_walk_evppi_uncertain_travel.pdf',height=10)#x11(height=10); 
par(mar=c(5,25,2,2)); 
