...
# How to run ITHIM-Simple

In this guide we explain how to install the ITHIM-Simple package, how to run the model using this package and how to produce summaries of the key results.

Please note that the guide is written for Windows 10 but should work similarly for different operating systems. We are using the latest R version - at the time of writing, which is `R version 4.3.3 (2024-02-29)`.
...
-   **orig_inj**: gives baseline average yearly injury counts by strike mode and casualty mode before accidents where strike mode = cas mode where converted to NOV accidents. There are two tables, one containing the original numbers from the original injury data and the other one  the original counts scaled by the injury_reporting_rate.
...
-   injury_tables.html: html files containing injury summary statistics of the respective model runs.
...
-   summary_tables.html: html files containing output summaries of the respective model runs.
...
-   summary_tables_PA.html: html files containing air pollution and physical activity output statistics of the respective model runs.
