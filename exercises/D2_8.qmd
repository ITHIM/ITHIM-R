...
You might have noticed that for some people (or `participants`), there is no change in their PA volume. In this exercise, we'd like to count all those people. Write the code below for it that counts such cases in each scenario.

```{r}

# The mmets dataset now contains ONLY those participants with travel behaviour
total_people_with_travel <- _(mmets_pp_total_wide)

# Calculate how many people haven't changed their baseline PA in the cycling scenario
count_unchanged_people_in_sc_cycle <- total_people_with_travel - 
  (mmets_pp_total_wide |> filter(baseline == _) |> nrow())

# Calculate the proportion using all people with travel behaviour
percentage_unchanged_people_in_sc_cycle <- round(_ / _ * 100, 2)
