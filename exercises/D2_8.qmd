...
```{r setup, echo=FALSE, warning=F, eval=TRUE}
#| echo: false
#| warning: false
#| eval: true
#| message: false
...
Just to remind you the trip dataset is stored as `trip_scens_sets` and it contains travel bahviour of baseline and all the scenarios.
...
In order to calculate the total travel related PA for each person, we need to use the following function: weekly duration in hours \* mMET hours per week. 
For cycling, mMET hours per week is `5.8` and for walking, it is `2.5`. In order to have a combined travel related mMET hours per week, we sum together PA volume for `cycling` and `walking` together.
