...
trips_ped_scen[trips_ped_scen$trip_distance_cat  == '_' & 
                 trips_ped_scen$trip_mode != "_"]$trip_mode <- '_'

# Do the same for `stage_mode` as well
trips_ped_scen[trips_ped_scen$trip_mode == "pedestrian"]$stage_mode <- '_'
...
trips_ped_scen[trips_ped_scen$trip_mode == "_"]$stage_duration <- trips_ped_scen[trips_ped_scen$trip_mode == "_"]$stage_distance * 60 / 2.5
...
ntrips <- _(trips_baseline, trips_ped_scen)
```
...
mmets_pp_total_wide <- mmets_pp_total |> 
  ...
  pivot_wider(names_from = scenario, values_from = _)
